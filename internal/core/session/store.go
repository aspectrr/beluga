...
func (s *Store) Create(ctx context.Context, source, sourceID string, metadata json.RawMessage) (*model.Session, error) {
	if metadata == nil {
	...
	err := s.pool.QueryRow(ctx, `
		INSERT INTO sessions (source, source_id, metadata)
		VALUES ($1, $2, $3)
		RETURNING id, source, source_id, status, sandbox_id, metadata, created_at, updated_at
	`, source, sourceID, metadata).Scan(
		&sess.ID, &sess.Source, &sess.SourceID, &sess.Status,
		&sess.SandboxID, &sess.Metadata,
...
func (s *Store) Get(ctx context.Context, id string) (*model.Session, error) {
	var sess model.Session
	err := s.pool.QueryRow(ctx, `
		SELECT id, source, source_id, status, sandbox_id, metadata, created_at, updated_at
...
// GetBySource retrieves a session by its source and source_id.
